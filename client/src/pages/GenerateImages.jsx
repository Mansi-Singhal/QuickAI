...
const GenerateImages = () => {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            setLoading(true)
            const prompt = `Generate an image of ${input} in the sty
             ${selectedStyle}`
    ...
    return (
        <div onSubmit={onSubmitHandler} className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
            ...
            <form className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
                ...
                <textarea onChange={(e) => setInput(e.target.value)} value={input} row={4} type="text" className='w-full p-2 mt-2 px-3 outline-none text-sm rounded-md border border-gray-300' placeholder='Describe what you want to see in the image...' required />
